...
- **Item 4 (RDS/API Gateway) não funciona em AWS Academy Lab**: exige criar uma IAM role com trust policy pra conta AWS do New Relic, e o Lab bloqueia `iam:CreateRole` — sem uma role pré-existente com essa trust policy pra reaproveitar (diferente das Lambdas/EKS, que reaproveitam a `LabRole`), essa parte específica fica indisponível no Lab. Os outros três itens (APM, Kubernetes, Lambda layer) não são afetados — nenhum deles exige criar IAM role nova. Ver README de `newrelic-aws-integration/` em `oficina-mecanica-lambda-auth`.

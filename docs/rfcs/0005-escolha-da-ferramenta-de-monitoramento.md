...
- **New Relic**: escolhido. Cobre o mesmo escopo (APM, infraestrutura, logs, alertas) com integrações nativas para Kubernetes/EKS, AWS Lambda (via layer/extension) e serviços AWS gerenciados como RDS e API Gateway (via CloudWatch Metric Streams). Mantém um **free tier perene** (não um trial) de 100 GB/mês de ingestão de dados e um usuário "full platform" sem exigir cartão de crédito — suficiente para o volume de telemetria gerado por este projeto e compatível com a conta do AWS Academy Lab.

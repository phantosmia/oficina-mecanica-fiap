# Terraform root separado (não módulo do ../aws) de propósito: o provider
# "newrelic" precisa de credenciais próprias (user API key, diferente da
# license key usada pelos agentes), e Terraform não permite count/for_each
# num "module" que declara sua própria configuração de provider ("legacy
# module"). Manter isso como um root à parte, aplicado manualmente quando
# alguém de fato quiser essa integração ativa, evita que ../aws (que todo
# mundo aplica, só pra ter o secret/IRSA da API) dependa de ter uma user
# API key configurada — mesmo padrão de "root Terraform separado e
# opcional" já usado neste repositório (infra/backend vs. infra/aws).
#
# Cobre RDS e API Gateway (integração de conta inteira, não escopada a um
# recurso específico — não importa que os dois vivam em repositórios
# diferentes: oficina-mecanica-infra-banco-dados e oficina-mecanica-
# lambda-auth). Fica aqui, e não em nenhum dos dois, porque este é o
# repositório "hub" do projeto — já concentra a documentação (RFCs/ADRs,
# incluindo o próprio ADR-0007) e o código que gera a telemetria de
# negócio (app/shared/telemetry.py) que os dashboards visualizam.
#

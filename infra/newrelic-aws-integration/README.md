...
Terraform root **separado** de [`../aws`](../aws) — liga a conta AWS ao New Relic para cobrir **RDS** e **API Gateway** via a integração de nuvem AWS do New Relic (item 4 do [ADR-0007](https://github.com/phantosmia/oficina-mecanica-fiap/blob/main/docs/adrs/0007-new-relic-como-plataforma-de-observabilidade.md)).

Vive aqui, não em `oficina-mecanica-infra-banco-dados` (dono do RDS) nem em `oficina-mecanica-lambda-auth` (dono do API Gateway) — a integração é de **conta inteira**, não escopada a um recurso específico, então não importa qual repositório a hospeda. Este é o repositório "hub" do projeto: já concentra a documentação (RFCs/ADRs, incluindo o próprio ADR-0007) e o código que gera a telemetria de negócio (`app/shared/telemetry.py`) que os dashboards visualizam — colocar aqui a estratégia de observabilidade de sistema inteiro evita espalhar essa decisão pelos repositórios mais estritamente escopados (Lambda, banco, Kubernetes).

## Por que um root separado, e não parte de `../aws`

O provider `newrelic` precisa de credenciais próprias (uma **user API key**, diferente da license key usada pelos agentes) e Terraform não permite `count`/`for_each` num `module` que declara sua própria configuração de provider ("legacy module"). Manter isso como um root à parte, aplicado manualmente só por quem de fato quiser essa integração ativa, evita que `../aws` (que todo mundo aplica, só pra ter o secret/IRSA da API) passe a exigir uma user API key configurada.
